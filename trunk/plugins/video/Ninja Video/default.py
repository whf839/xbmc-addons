...
#Thanks to Unbehagen for the decryption code NVDecode.py & the Flyupload code & Creating plugins in the first place
#& Coolblaze for the Megavideo algo & Jeroen for the Thumbnail.
#A collaboration of goodwill. Merry Christmas one and all.
...
def vidlinks(url,name):
        ...
        if name.find('Flash')>0:
                ...
                try:
                        ...
                        match=re.compile('\r\nData: (.+?)\r\n').findall(link)
                        method=re.compile('\r\nMethod: (.+?)\r\n').findall(link)
                        if match[0].find('flyupload')>0:#Thanks to Unbehagen.
