...
__plugin__ = "reeplay.it StandaloneTest"
...
DIR_RESOURCES = '/'.join( [DIR_HOME, "resources"] )
DIR_RESOURCES_LIB = '/'.join( [DIR_RESOURCES, "lib"] ) # shares modules with script version
...
try:
	...
	print e
	xbmcgui.Dialog().ok("XBMC Language Error", "Install a new XBMC build to run this script.", e)
	...
	def __init__( self, *args, **kwargs ):
		...
		if ( sys.argv[ 2 ] ):
			...
			if paramDict.has_key(self.PARAM_INFO):
				fn = DIR_RESOURCES+"/" + paramDict[self.PARAM_INFO]
