...
class UpdatePlugin:
	...
	def __init__( self, language, pluginName, pluginType ):
		...
		local_base_dir = xbmc.translatePath( "/".join( ['Q:','plugins', pluginType] ) )
		self.local_dir = "/".join( [local_base_dir, pluginName] )
		self.backup_base_dir = "/".join( [local_base_dir,'.backups'] )
		self.local_backup_dir = "/".join( [self.backup_base_dir, pluginName] )
...
if __name__ == "__main__":
	...
	try:
		...
		up = Update(xbmc.Language( lang_path ).getLocalizedString, pluginName, pluginType)
		up.removeOriginal()
