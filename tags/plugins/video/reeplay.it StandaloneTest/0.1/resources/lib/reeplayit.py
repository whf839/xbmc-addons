...
class ReeplayitLib:
	...
	def __init__(self, user, pwd, pageSize=50, highVQ=True, docType='xml'):
		...
		self.plsListItems = []
		self.videoListItems = []
		self.DEFAULT_THUMB_IMG = "/".join( [DIR_HOME, "default.tbn"] )
	...
	def getPlaylists(self):
		...
		if not self.plsListItems:
			...
			docFN = "/".join( [DIR_CACHE, os.path.basename(self.URL_PLAYLISTS)] )
			data = readFile(docFN)
			if not data:
			...
			if data:
				...
				for item in items:
					try:
						...
						imgFN = "/".join( [DIR_CACHE, imgName] )
						if not fileExist(imgFN):
							data = self.retrieve(imgURL, fn=xbmc.translatePath(imgFN))
							if data == "": # aborted
								break
							elif not data:
						...
						imgFN = xbmc.makeLegalFilename(imgFN)
			...
			if items:
				...
				for itemCount, item in enumerate(items):
					try:
						...
						imgFN = "/".join( [DIR_CACHE, imgName] )
						if not fileExist(imgFN):
							result = self.retrieve(imgURL, fn=xbmc.translatePath(imgFN))
							if result == "": # aborted
								break
							elif not result:
								imgFN = self.DEFAULT_THUMB_IMG
...
#			infoUrl = li.getProperty(self.PROP_URL)
	...
	def doesVideoExist(self, id):
		...
		exts = ('.ts','.mp4')
		for ext in exts:
			fn = "/".join( [DIR_CACHE, "%s_%s%s" % (id, self.vqProfile, ext)] )
			if fileExist(fn):
...
def showTextFile(filename):
    ...
    try:
        xmlFn = "DialogScriptInfo.xml"
        tbd = TextBoxDialogXML(xmlFn, DIR_HOME)
