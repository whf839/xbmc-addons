    ...
    def _get_forecast( self, htmlSource ):
        ...
        if ( len( days ) ):
            ...
            for count, day in enumerate( days ):
                ...
                self.forecast += [ ( day, iconpath, brief[ count ], temperature[ count ][ 0 ], _localize_unit( temperature[ count ][ 1 ] ), precip_title[ count ], precip_amount[ count ], outlook[ count ].strip(), daylight[ count ].split( ": " )[ 0 ], _localize_unit( daylight[ count ].split( ": " )[ 1 ], "time" ), ) ]
...
[^>]+>[^>]+>([^<]+)<"
        ...
        if ( len( headings ) ):
            for count, ( day, date, alert, ) in enumerate( headings ):
                ...
                lows[ count ][ 0 ], _localize_unit( lows[ count ][ 1 ] ), precips[ count * 2 ], precips[ count * 2 + 1 ].replace( "<B>", "" ).replace( "</B>", "" ),
                winds[ count ][ 0 ], winds[ count ][ 1 ], uvs[ count ][ 0 ], uvs[ count ][ 1 ],
                humids[ count ][ 0 ], humids[ count ][ 1 ], daylights[ count * 2 ][ 0 ], _localize_unit( daylights[ count * 2 ][ 1 ], "time" ), daylights[ count * 2 + 1 ][ 0 ],
...
class WeatherClient:
    ...
    BASE_MAPS = (
                                ...
                                ( "Weather Details (Doppler Radar)", "/maps/maptype/dopplerradarusnational/index_large.html", "Doppler\nRadar", ),
    ...
    def fetch_images( self, map ):
        try:
            base_path = ""
            expires = -1
            # TODO: finally determine if we want an animated setting (i think not)
            animated = True
            ##if ( not DEBUG ):
