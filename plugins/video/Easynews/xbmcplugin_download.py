...
pDialog.create( "EasyNews Downloader " )
...
class Main:
	...
	self.startTime = 0
        try:
            ...
            if ( not os.path.isfile( filepath ) ):
                # fetch the video
		print "LINK URL: "+g_movie_url.replace("http://", "http://"+self.settings[ "username" ]+":"+self.settings[ "password" ]+"@")
		self.startTime = time.time()
                urllib.urlretrieve( g_movie_url.replace("http://", "http://"+self.settings[ "username" ]+":"+self.settings[ "password" ]+"@"), tmp_path, self._report_hook )
	...
	totalCount = count * (blocksize/1024)
	sizeLeft = (totalsize/1024) - totalCount
	speed = totalCount / (time.time() - self.startTime)

	timeLeftHr = int(round((sizeLeft / (speed +.00001))/3600,0))
	timeLeftMin = int(round((((sizeLeft / (speed +.00001))/3600)%1)*60,0))
	timeLeft = str(timeLeftHr)+"hr"+str(timeLeftMin)+"m"
	...
	msg3= str(round(speed, 2))+"KBs  Est Time Left: "+str(timeLeft)
        pDialog.update( percent, msg1, msg2, str(msg3) )
