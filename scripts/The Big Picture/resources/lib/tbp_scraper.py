...
class TBP:
    ...
    def getHTML(self, url, headers = [('User-Agent', 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US; rv:1.8.1.14) Gecko/20080404 Firefox/2.0.0.14')]):
        ...
        try:
            print '[SCRIPT] %s attempting to open %s' % (__name__, url)
            usock = opener.open(url)
            response = usock.read()
            usock.close()
            return response
        except urllib2.HTTPError, error:
            print 'error:\n', error.code, error.msg, error.geturl()
    ...
    def getAlbums(self, url):
        ...
        for node in storyNodes:
            ...
            link = node.h2.a['href']
            description = self.cleanHTML(node.span.div.contents)
            pic = node.span.img['src']
    ...
    def getPhotos(self, url):
        ...
        tree = BeautifulSoup(self.getHTML(url))
        title = tree.findAll('div', 'headDiv2')[0].h2.a.string
        self.photos = list()
        photoNodes = tree.findAll('div', {'class': re.compile('bpImageTop|bpBoth')})
        for node in photoNodes:
            pic = node.img['src']
            description = self.cleanHTML(node.findAll('div', 'bpCaption')[0])            
