...
from ce_playlist import _get_thumbnail

__useragent__ = "QuickTime/7.2 (qtver=7.2;os=Windows NT 5.1Service Pack 3)"


class Main:
    xbmc.log( "%s - XBMC Movie Library Trailer Scraper" % logmessage, xbmc.LOGNOTICE )
    ...
    def fetch_trailers( self ):        
        ...
        xbmc.log( "%s  - SQL: %s" % ( logmessage, sqlquery, ), xbmc.LOGNOTICE )
        try:
            sqlresult = xbmc.executehttpapi( "QueryVideoDatabase(%s)" % quote_plus( sqlquery ), )
            #xbmc.log( "%s - sqlresult: %s" % sqlresult, xbmc.LOGNOTICE )
            ...
            for trailer in trailer_list:
                ...
                if self.settings[ "trailer_limit_genre" ] and ( not list(set(trailer_genre) & set(self.genre) ) ):
                    xbmc.log("%s - Genre Not Matched - Skipping Trailer" % logmessage, xbmc.LOGNOTICE )
                    continue
                if self.settings[ "trailer_limit_mpaa" ] and ( not trailer_ratingsql or not trailer_ratingsql == self.mpaa ):
                    xbmc.log("%s - MPAA Not Matched - Skipping Trailer" % logmessage, xbmc.LOGNOTICE )
                    continue
                trailer_info = ( xbmc.getCacheThumbName( trailer_path ), # id
                                 title, # title
                                 trailer_path, # trailer
                                 _get_thumbnail( trailer_path ), # thumb
                                 '', # plot
                                 '', # runtime
            ...
            xbmc.log( "%s - Error searching database", xbmc.LOGNOTICE )
            traceback.print_exc()

    def _get_watched( self ):
        xbmc.log("%s - Getting Watched List" % logmessage, xbmc.LOGNOTICE )
        try:
    ...
    def _reset_watched( self ):
        xbmc.log("%s - Resetting Watched List" % logmessage, xbmc.LOGNOTICE )
        base_path = os.path.join( self.BASE_CURRENT_SOURCE_PATH, self.settings[ "trailer_scraper" ] + "_watched.txt" )
    ...
    def _save_watched( self ):
        xbmc.log("%s - Saving Watched List" % logmessage, xbmc.LOGNOTICE )
        try:
            # base path to watched file
            base_path = os.path.join( self.BASE_CURRENT_SOURCE_PATH, self.settings[ "trailer_scraper" ] +"_watched.txt" )
